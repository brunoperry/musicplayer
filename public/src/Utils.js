class Utils {
  ...
  static getIcon(iconName, animName = null) {
    ...
    const ico = icons.find(
      (btn) => `icon-${iconName}` === `${btn.className.baseVal}`
    );
    animName
  ...
  static getButton(buttonType) {
    const btns = [
      ...document.querySelector("#buttons-template").content.cloneNode(true)
        .children,
    ];
    const btn = btns.find(
  ...
  static setStartTouchOrMouseEvent(elem, callback) {
    if (this.IS_MOBILE)
      elem.addEventListener(
        "touchstart",
        (e) => callback({ x: e.touches[0].clientX, y: e.touches[0].clientY }),
        { passive: true }
      );
    else
  ...
  static setMoveEvent(elem, callback) {
    ...
    this.IS_MOBILE
      ? (elem.ontouchmove = (ev) => {
          callback({
            x: ev.touches[0].clientX - w,
            y: ev.touches[0].clientY - h,
          });
        })
      : (elem.onmousemove = (ev) => {
          callback({ x: ev.clientX - w, y: ev.clientY - h });
        });
  }
  static unsetTouchOrMouseEvents(elem) {
    if (this.IS_MOBILE)
      elem.ontouchstart, elem.ontouchend, (elem.ontouchmove = null);
  ...
  static async copyTextToClipboard(text) {
    if (!navigator.clipboard) {
      console.error("ERROR!", "No clipboard support");
      return;
    }
    try {
  ...
  static Translate(elem, vec) {
    return this.#doTransform(elem, `translate3D(${vec.x},${vec.y},0)`);
  }
  static Rotate(elem, val) {
  ...
  static Spin(elem, deg = "0") {
    return this.#doTransform(elem, `scale3D(1,1,1) rotate(${deg}deg)`);
  }
  static Pop(elem) {
    return this.#doTransform(elem, "scale3D(1,1,1)");
  }
}
Utils.init();
