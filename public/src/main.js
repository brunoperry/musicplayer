...
menu.addEventListener(Menu.ACTION, async (ev) => {
  switch (ev.detail.action) {
    ...
    case "play":
      ...
      player.play(
        listData,
        listData.findIndex((itm) => itm.id === detail.currentItem.id)
      );
      break;
    case "closing":
...
player.addEventListener(Player.ACTION, (ev) => {
  switch (ev.detail.type) {
    ...
    case Player.Actions.PLAYING:
      ...
      let url = "?path=none";
      if (player.currentTrack.type !== "file")
        url = `?path=${player.currentTrack.path}`;
      history.URL(url);
...
player.currentTrack
  ? (info.title = player.currentTrack.title)
  : (info.title = "Music Player");
