...
export default class Menu extends Component {
    ...
    constructor(data) {
        ...
        this.#breadcrumb.addEventListener(Breadcrumb.GO_TO, ev => this.gotoList(ev.detail));
        this.#menuButton = new IconButton(['menu', 'cross'], 'menu-button');
    ...
    addList(listData, andShow=true) {
        ...
        list.addEventListener(List.CLICK, ev => {
            ...
            else if(data.action) this.dispatchEvent(Menu.ACTION, {detail:{
                action:data.action,
                listData:list.data,
                currentItem: data
            }})
    ...
    async gotoList(index=0) {
        ...
        let rLists = [];
        for (let i = this.#lists.length-1; i > index; i--) { rLists.push(this.#lists[i]) }
        this.#currentList = this.#lists[index];
        this.#currentList.peekX('0');
        rLists.forEach(lst => lst.hide());
        
        this.#lists.length - rLists.length > 1 ? this.#breadcrumb.show() : this.#breadcrumb.hide();
    ...
    static get ACTION() {return 'menuaction'}

    set activated(val) {this.#isActivated = val}
    get activated() {return this.#isActivated}

    get currentData() {return this.#currentList.currentItem.data;}
}